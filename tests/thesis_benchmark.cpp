...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS) schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)query_num; i++) {
      if (search_mode == PIPE_SEARCH) {
        ...
        index.beam_search(query + (i * query_dim), recall_at, mem_L, L,
                          query_result_tags + (i * recall_at),
                          query_result_dists + (i * recall_at),
                          beam_width, stats + i, nullptr, false);
      }
    ...
    float recall = 0;
    if (gt_ids != nullptr) {
      recall = pipeann::calculate_recall(query_num, gt_ids, nullptr, gt_dim,
    ...
    // 计算平均统计
    double mean_ios = pipeann::get_mean_stats(stats, query_num,
        [](const pipeann::QueryStats& s) { return s.n_ios; });
    double avg_compute = pipeann::get_mean_stats(stats, query_num,
        [](const pipeann::QueryStats& s) { return s.compute_phase_us; });
    double avg_prefetch = pipeann::get_mean_stats(stats, query_num,
        [](const pipeann::QueryStats& s) { return s.prefetch_phase_us; });
    double avg_io_amp = pipeann::get_mean_stats(stats, query_num,
        [](const pipeann::QueryStats& s) { return s.io_amplification; });
    double avg_overlap = pipeann::get_mean_stats(stats, query_num,
        [](const pipeann::QueryStats& s) { return s.overlap_ratio; });
    
    ofs << L << "," << recall << "," << qps << "," 
        << avg_lat << "," << p50_lat << "," << p90_lat << "," 
        << p95_lat << "," << p99_lat << "," << mean_ios << ","
        << avg_compute << "," << avg_prefetch << ","
        << avg_io_amp << "," << avg_overlap << "\n";
    
    std::cout << "L=" << L << " Recall=" << recall << " QPS=" << qps 
              << " AvgLat=" << avg_lat << "us P99=" << p99_lat << "us\n";
 ...
 */
template<typename T, typename TagT = uint32_t>
void run_io_amplification_experiment(
    pipeann::SSDIndex<T, TagT>& index,
    T* query,
    size_t query_num,
    size_t query_dim,
 ...
 */
template<typename T, typename TagT = uint32_t>
void run_pipeline_width_experiment(
    pipeann::SSDIndex<T, TagT>& index,
    T* query,
    size_t query_num,
    size_t query_dim,
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS) schedule(dynamic, 1)
    ...
    ofs << bw << "," << qps << "," << recall << ","
        << avg_lat << "," << p99_lat << "," << mean_ios << "," << io_efficiency << "\n";
    
    std::cout << "BW=" << bw << " QPS=" << qps << " Recall=" << recall 
              << " AvgLat=" << avg_lat << "us IoEff=" << io_efficiency << "%\n";
...
int main(int argc, char** argv) {
  ...
  if (argc > 7) num_threads = std::atoi(argv[7]);
  if (argc > 8) beam_width = std::atoi(argv[8]);
  if (argc > 9) recall_at = std::atoi(argv[9]);
  if (argc > 10) mem_L = std::atoi(argv[10]);
  ...
  if (data_type == "float") {
    ...
    std::unique_ptr<pipeann::SSDIndex<float>> index(
        new pipeann::SSDIndex<float>(metric, reader, nbr_handler, true));
    ...
    switch (exp_type) {
      case 1: {
        ...
        run_latency_experiment(*index, query_f, query_num, query_dim,
                               recall_at, mem_L, L_values, beam_width,
                               output_file, gt_ids, gt_dim);
        break;
      }
      case 5:
        run_io_amplification_experiment(*index, query_f, query_num, query_dim,
                                         recall_at, mem_L, 50, beam_width, output_file);
        break;
      case 6: {
        std::vector<uint32_t> bw_values = {1, 2, 4, 8, 16, 32, 64};
        run_pipeline_width_experiment(*index, query_f, query_num, query_dim,
                                       recall_at, mem_L, 50, bw_values,
                                       output_file, gt_ids, gt_dim);
        break;
      }
      case 7:
    ...
    switch (exp_type) {
      case 1: {
        std::vector<uint64_t> L_values = {10, 20, 30, 40, 50, 60, 80, 100, 150, 200};
        run_latency_experiment(*index, query_u8, query_num, query_dim,
                               recall_at, mem_L, L_values, beam_width,
                               output_file, gt_ids, gt_dim);
        break;
      }
      case 5:
        run_io_amplification_experiment(*index, query_u8, query_num, query_dim,
                                         recall_at, mem_L, 50, beam_width, output_file);
        break;
      case 6: {
        std::vector<uint32_t> bw_values = {1, 2, 4, 8, 16, 32, 64};
        run_pipeline_width_experiment(*index, query_u8, query_num, query_dim,
                                       recall_at, mem_L, 50, bw_values,
                                       output_file, gt_ids, gt_dim);
        break;
      }
  ...
  // 清理
  if (query_f) delete[] query_f;
  if (query_u8) delete[] query_u8;
  if (query_i8) delete[] query_i8;
