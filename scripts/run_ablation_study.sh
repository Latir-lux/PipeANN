...
prepare_exp1_data() {
  ...
  # Ablation study 使用独立的GT文件（不与run_system_comparison.sh冲突）
  ...
  local exp1_gt_file="${gt_prefix}_ablation_base${base_pct}_k${exp1_gt_k}.${gt_ext}"
...
PY
  ...
  if [ "${FORCE_REGEN_EXP1_GT}" = "1" ] && [ -f "${exp1_gt_file}" ]; then
    echo "Removing existing ablation GT to force regeneration: ${exp1_gt_file}" >&2
    rm -f "${exp1_gt_file}"
  fi

  if [ -f "${exp1_gt_file}" ]; then
    echo "Using existing ablation GT: ${exp1_gt_file}" >&2
  else
    # 为日志输出计算百分比值（0p8 -> 80）
    local base_pct_display
    base_pct_display=$(python3 -c "print(int(round(${base_ratio} * 100)))")
    echo "Generating ablation GT for base ${base_pct_display}% (${base_pts_count} points), K=${exp1_gt_k}..." >&2
    ...
    if [ $? -ne 0 ]; then
      echo "Error: Failed to generate ablation GT" >&2
      exit 1
    fi
  fi

  EXP1_GT_FILE=${exp1_gt_file}
  echo "Ablation GT file: ${EXP1_GT_FILE}" >&2
}
...
run_exp1_clustering() {
  ...
  if [ ! -f "${EXP1_GT_FILE}" ]; then
    echo "Error: ablation GT file not found: ${EXP1_GT_FILE}" >&2
    echo "It should have been generated by prepare_exp1_data" >&2
    exit 1
  fi
